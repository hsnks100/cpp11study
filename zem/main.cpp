...
int main() {
   ...
   */
   ...
   /* About Capture */
   /* for copy all values, use [=] or [ <value> ] */
   setInit(a, b, c);
   [=]() {
   ...
   /* for reference all values, use [&] or [ &<value> ] */
   setInit(a, b, c);
   [&]() {
      a = 100; b = 200; c = 300;
   ...
   shared_ptr<TestForSharedPtr> shdptr1(tc1);
   shared_ptr<TestForSharedPtr> shdptr2(tc2);
   shdptr1.swap(shdptr2);
   vector< shared_ptr<TestForSharedPtr> > shdptrVector;
   shdptrVector.push_back(shdptr1);
   shdptrVector.push_back(shdptr2);
   cout << shdptr1.get()->cv << " " << shdptr1.get()->iv << endl;

   unique_ptr<TestForSharedPtr> unqptr1(new TestForSharedPtr(1));
   unique_ptr<TestForSharedPtr> unqptr2(new TestForSharedPtr(2));

   vector< unique_ptr<TestForSharedPtr> > unqptrVector;
   ...
   tuple<int, int, int, int> testTuple;
   for (int i = 0; i < tuple_size<decltype(testTuple)>::value; i++) {
      // get<i>(testTuple) = i; // this cannot
   }
   get<0>(testTuple) = 0;
   get<1>(testTuple) = 1;
   get<2>(testTuple) = 2;
   ...
   tuple<char, int> tuple1;
   tuple<short, double> tuple2;

   auto tuple3 = tuple_cat(tuple1, tuple2);
   // auto : tuple<char, int, short, double>
